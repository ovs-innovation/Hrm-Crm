    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage company projects, clients, and statuses.</p>
        </div>
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className={`px-3 py-1 text-xs font-bold rounded-full ${
                  project.priority === 'High' ? 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400' :
                    ...
                    <div className="flex -space-x-2">
                      {project.team.slice(0, 4).map((emp, i) => (
                        <div key={i} className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 border-2 border-white dark:border-slate-900 flex items-center justify-center text-xs font-bold text-slate-600 dark:text-slate-300" title={emp.name}>
                          {emp.profilePicture ? (
                            <img src={`${backendBaseUrl}${emp.profilePicture}`} alt={emp.name} className="w-full h-full rounded-full object-cover" />
                          ) : emp.name.charAt(0)}
                        </div>
                      ))}
