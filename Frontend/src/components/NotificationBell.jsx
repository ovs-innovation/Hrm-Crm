...
const NotificationBell = () => {
  ...
  return (
    <div className="relative" ref={ref}>
      <button type="button" onClick={() => { setOpen(!open); if (!open) load(); }} className="relative rounded p-1.5 text-muted hover:bg-soft" aria-label="Notifications">
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-1 w-80 rounded border border-line bg-surface shadow-lg">
          <div className="border-b border-line px-3 py-2 text-[13px] font-semibold text-ink">Notifications</div>
          <div className="max-h-80 overflow-y-auto">
            {items.length === 0 ? (
              <p className="px-3 py-6 text-center text-[13px] text-muted">No notifications</p>
            ) : items.map((n) => (
              <button key={n._id} type="button" onClick={() => markRead(n._id, n.link)} className={`block w-full border-b border-line px-3 py-2.5 text-left last:border-0 hover:bg-soft ${n.read ? 'opacity-70' : ''}`}>
