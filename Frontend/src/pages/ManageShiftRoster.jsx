...
const ManageShiftRoster = () => {
  ...
  return (
    <PageShell
      title="Shift roster"
      description="Assign and manage employee shift schedules"
      count={items.length}
      actions={
        <>
        ...
        <table className="w-full min-w-[700px] text-left text-[13px]">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-10 text-center text-muted">No shifts for this month.</td></tr>
            ) : items.map((s) => (
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/30 p-4">
          <div className="w-full max-w-md rounded border border-line bg-surface">
            <div className="border-b border-line px-4 py-3 font-semibold text-[15px] text-ink">Assign shift</div>
            <form onSubmit={handleSubmit} className="space-y-3 p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="app-label mb-1 block text-[13px]">Shift</label><select className="app-input h-9 w-full text-[13px]" value={form.shiftType} onChange={(e) => setForm({ ...form, shiftType: e.target.value })}>{SHIFTS.map((s) => <option key={s}>{s}</option>)}</select></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
...
export default ManageShiftRoster;
